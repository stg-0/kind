...
For API Server exposure, a load balancer is created with name `<cluster_id>-apiserver` and port 6443 accessible by the public network (the assigned public IP is the same as configured in the _Kubeconfig_) and one _Target group_ with the corresponding _control-plane_ nodes.
